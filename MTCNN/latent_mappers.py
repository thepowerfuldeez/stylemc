...
class HairMapper(Module):
    ...
    def forward(self, x, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor):
        if hairstyle_text_inputs.shape[1] != 1:
            hairstyle_embedding = self.clip_model.encode_text(hairstyle_text_inputs).unsqueeze(1).repeat(1, 18,
                                                                                                         1).detach()
        elif hairstyle_tensor.shape[1] != 1:
            hairstyle_embedding = self.gen_image_embedding(hairstyle_tensor, self.clip_model,
                                                           self.preprocess).unsqueeze(1).repeat(1, 18, 1).detach()
        else:
        ...
        if color_text_inputs.shape[1] != 1:
            ...
            color_embedding = self.gen_image_embedding(color_tensor, self.clip_model, self.preprocess).unsqueeze(
                1).repeat(1, 18, 1).detach()
        else:
